<template>
  <section class="pd-bottom46">
    <section class="am-flexbox pd15 am-pt-10 border-top border-bottom am-pb-5">
      <div class="am-flexbox-item am-right-10">
        <select class="con-select" v-model="P7043.stock_account" @change="changeStockAccount">
          <option v-for="option in RS4070"  v-bind:value="option.stock_account">
            {{$dic('exchange_type', option.exchange_type)}}—{{P7043.stock_account = option.stock_account}}
          </option>
        </select>
        <div class="con-code">
          <div class="ui-formContainer btn-border">
               <span class="ui-form-field">
                  <input v-model="P7043.stock_code" debounce="700" type="number" class="text_input" defaultvalue="股票名称/代码" value="" placeholder="请输入股票代码">
                  <a v-show="P7043.stock_code > 0" @click="P7043.stock_code=''" class="close-ico bottom-10"></a>
              </span>
            <span class="share_name am-ft-gray">{{P7043.stock_name}}</span>
          </div>
          <!--浮出页面-->
          <div class="list-codeSug" v-show="(StockList.length > 0)">
            <ul class="select-results am-selectList" v-for="item in StockList">
              <li @click="P7043.stock_code = item.stock_code">
        ...
        <div class="am-flexbox ui-formContainer am-flexbox-remind btn-border firmb-flexbox-buy am-mt-10 am-bg-white">
          <div class="firmbTags firmbTags-minus am-left-10"  @click="P7043.entrust_price = !isNaN(P7043.entrust_price) ? ((parseFloat(P7043.entrust_price) - (parseInt(RS1050.price_step) * 0.001)).toFixed(RS1050.price_step === '10' ? 2 : 3)) : 1">
            <p class="p1"></p>
          </div>
          <div class=" borderNo am-flexbox-item">
                    <span class="ui-form-field">
                        <input v-model="P7043.entrust_price" type="number" class="text_input am-ft-center " value="" placeholder="卖出价">
                    </span>
          </div>
          <div class="firmbTags am-right-10" @click="P7043.entrust_price = !isNaN(P7043.entrust_price) ? ((parseFloat(P7043.entrust_price) + (parseInt(RS1050.price_step) * 0.001)).toFixed(RS1050.price_step === '10' ? 2 : 3)) : 1">
            <p class="p2"></p>
          </div>
        </div>
        <select class="con-select" v-model="P7043.entrust_prop">
          <template v-if="P7043.exchange_type === '1'">
            <option value="0">限价委托</option>
            <option value="U">五档即成剩撤</option>
            <option value="R">五档即成转限价</option>
          </template>
          <template v-if="P7043.exchange_type === '2'">
            <option value="0">限价委托</option>
          ...
          <template v-if="P7043.exchange_type === ''">
            <option value="0">限价委托</option>
          </template>
        </select>
        <div class="am-flexbox am-ft-sm" style="display:none">
          <div class="am-list-item am-flexbox-item am-flex2" am-mode="check" style="min-width:80px;">
            <div class="am-checkbox" am-mode="middle">
              <input type="checkbox" v-model="P7043.serial_no" />
              <span class="icon-check am-checkLeft" am-mode="icon-check-left"></span>
              <p class="am-ml-20">指定合约</p>
            </div>
          </div>
          <!--点击指定合约出现-->
          <div  v-show="P7043.serial_no" class="am-ft-right overflow-space"><span>{{P7043.serial_no === true ? '' : P7043.serial_no}}</span> </div>
          <div  v-show="!P7043.serial_no" class="am-flexbox-item am-ft-right"><a href="">顺序还款</a> </div>
        </div>
        <div class="am-flexbox ui-formContainer am-flexbox-remind firmb-flexbox-buy am-mt-10 btn-border am-bg-white">
          <div class=" borderNo am-flexbox-item">
                    <span class="ui-form-field">
                        <input v-model="P7043.entrust_amount" type="number" class="text_input am-ft-center " value="" placeholder="委托数量">
                    </span>
          </div>
        </div>
        ...
        <div class="am-flexbox am-mt-10">
          <div class=" borderNo am-flexbox-item am-ft-lightgray am-ft-smmm">应还：{{RS7050.need_return_balance ?  Math.transCNScientific(parseInt(RS7050.need_return_balance)) : '--'}}</div>
          <div class="am-right-10 am-ft-lightgray am-ft-smmm">可卖：{{RS7000.enable_amount ? Math.transCNScientific(parseInt(RS7000.enable_amount)) : '--'}}</div>
        </div>
        <a @click="AS7043(this)" class="am-button am-mt-10 am-mb-10" am-mode="green">卖出</a>
      </div>
      <div class="sell-26" style="min-width: 95px">
        <ul class=" am-ft-smm border-bottom am-pb-20 sell-5 sell-8 am-relative">
          <li @click="this.P7043.entrust_price = this.RealData.sellPrice5" class="am-flexbox">
          ...
          <li @click="this.P7043.entrust_price = this.RealData.sellPrice4" class="am-flexbox">
          ...
          <li @click="this.P7043.entrust_price = this.RealData.sellPrice3" class="am-flexbox">
          ...
          <li @click="this.P7043.entrust_price = this.RealData.sellPrice2" class="am-flexbox">
          ...
          <li @click="this.P7043.entrust_price = this.RealData.sellPrice1" class="am-flexbox">
        ...
        <ul class="am-ft-smm am-mt-20 sell-5 sell-8">
          <li @click="this.P7043.entrust_price = this.RealData.buyPrice1" class="am-flexbox">
          ...
          <li @click="this.P7043.entrust_price = this.RealData.buyPrice2" class="am-flexbox">
          ...
          <li @click="this.P7043.entrust_price = this.RealData.buyPrice3" class="am-flexbox">
          ...
          <li @click="this.P7043.entrust_price = this.RealData.buyPrice4" class="am-flexbox">
          ...
          <li @click="this.P7043.entrust_price = this.RealData.buyPrice5" class="am-flexbox">
    ...
    <ul class="itme border-bottom" v-el:wrapper_header  >
    ...
    <div class="container-list page-infinite-wrapper" v-el:wrapper :style='{ height: wrapper.height + "px"  }'>
      <ul class="itme am-cells" v-infinite-scroll='loadMore()' infinite-scroll-disabled='loading' infinite-scroll-distance='50'>
        <li class="am-flexbox border-bottom am-pd-15" v-for="item in RS4030" debounce="200" v-bind:class="{ 'high_light': P7043.stock_code === item.stock_code }" @click="this.P7043.stock_code = item.stock_code">
          <div class="am-flexbox-item">
...
<script>
  ...
  export default{
    created () {
      ...
      // 监听市场类别
      this.$watch('P7043.exchange_type', function (val, oldVal) {
        if (val === '') { // 没有市场类别
          // this.P7043['stock_account'] = ''
        } else {
          this.RS4070.map((account) => { // 信用账户
            if (account['exchange_type'] === val) {
              this.P7043['stock_account'] = account['stock_account']
            }
          })
        }
      })
      // 监听股票代码
      this.$watch('P7043.stock_code', function (val, oldVal) {
        this.StockList = []
        this.RealData = {}
        if (val && val.length === 6) { // 输入完成
            ...
            if ((list instanceof Array) && list.length === 1) {
              if (!list[0]['exchange_type']) {
                ...
                $Market(val).then((obj) => {
                  this.RealData = obj
                  // 买入价
                  this.P7043.entrust_price = obj.buyPrice1
                  // 设置要查询可卖数量的股票代码、委托价格、市场类别
                  this.P7000['stock_code'] = val
                  this.P7000['exchange_type'] = this.P7043.exchange_type
                  this.P7000['entrust_price'] = this.P7043.entrust_price
                })
                this.P1050['stock_code'] = val
                this.AS1050()
                this.P7043.exchange_type = list[0]['exchange_type']
                this.P7043.stock_name = list[0]['stock_name']
              }
            }
          })
        } else {
          ...
          // 重置内容
          this.P7043['exchange_type'] = ''
          this.P7043['stock_name'] = ''
          this.P7043['entrust_price'] = ''
          this.P7043['entrust_amount'] = ''
          // 重置要查询可还数量的股票代码
          this.P7000['stock_code'] = ''
        }
      })

      // 监听根据股票代码查询 - 可买数量
      this.$watch('P7000.stock_code', function (val, oldVal) {
        if (val === '') {
          this.AS7000RESET()
        } else {
          this.AS7000()
      ...
      this.$watch('P7043.entrust_amount', function (val, oldVal) {
        // 文本框 val 可还 this.RS7000.enable_amount
        this.sellToolActive = 0
        let amount = this.RS7000.enable_amount
      ...
      // 监听复选框指定合约
      this.$watch('P7043.serial_no', function (val, oldVal) {
        if (val && val === true) { // 表示选中
          this.P7061['fund_account'] = '991600643'
          // this.P7061['stock_account'] = '1'
          this.P7061['stock_code'] = '600030'
          this.AS7061()
        }
      })
      // 监听列表内容是否发生变化
    ...
    ready () {
      this.wrapper.height = document.documentElement.clientHeight - 50
      var _self = this
      window.onscroll = function (e) {
        if (_self.$route.path === '/financing/ticket-payments' && document.body.scrollTop >= 382) { //
          window.scrollTo(0, 382)
        }
      }
      var startY = 0
      this.$els.wrapper_header.addEventListener('touchstart', function (e) {
        startY = e.touches[0].pageY
      })

      this.$els.wrapper_header.addEventListener('touchmove', function (e) {
        if (startY > 0) {
          if (e.touches[0].pageY > startY) {
            window.scrollTo(0, (382 - (e.touches[0].pageY - startY)))
            startY = 0
            return false
          }
        }
    ...
    vuex: {
      getters: {
        RS7000: state => state.all['700:0'],
        RS7050: state => state.all['705:0'],
        RS7043: state => state.all['704:3'],
    ...
    methods: {
      ...
      sellTools (m) {
        ...
        if (!isNaN(amount)) {
          let result = (amount / m)
          this.P7043.entrust_amount = (result <= 100) ? 100 : result - (result % 100)
        }
      },
      changeStockAccount () {
        // 重置内容
