  ...
  <section class="pd-bottom46">
    <section class="am-flexbox pd15 am-pt-10 border-top border-bottom am-pb-5 header-top am-bg-l">
      <div class="am-flexbox-item am-right-10">
        <div class="con-code">
          <div class="ui-formContainer btn-border">
                     <span class="ui-form-field">
                        <input v-model="P3020.stock_code" debounce="700" type="number" class="text_input" defaultvalue="股票名称/代码" value="" placeholder="请输入股票代码">
                        <a v-show="P3020.stock_code" @click="P3020.stock_code=''" class="close-ico bottom-10"></a>
                    </span>
            <span class="share_name am-ft-gray">{{P3020.stock_name}}</span>
          </div>
          <!--浮出页面-->
          <div class="list-codeSug" v-show="(StockList.length > 0)">
            <ul class="select-results am-selectList" v-for="item in StockList">
              <li @click="P3020.stock_code = item.stock_code">
        ...
        <div class="am-flexbox ui-formContainer am-flexbox-remind btn-border firmb-flexbox-buy am-mt-10 am-bg-white">
          <div class="firmbTags firmbTags-minus am-left-10"  @click="P3020.entrust_price = !isNaN(P3020.entrust_price) ? ((parseFloat(P3020.entrust_price) - (parseInt(RS1050.price_step) * 0.001)).toFixed(RS1050.price_step === '10' ? 2 : 3)) : 1">
            <p class="p1"></p>
          </div>
          <div class=" borderNo am-flexbox-item">
                    <span class="ui-form-field">
                        <input v-model="P3020.entrust_price" type="number" class="text_input am-ft-center " value="" placeholder="买入价">
                    </span>
          </div>
          <div class="firmbTags am-right-10" @click="P3020.entrust_price = !isNaN(P3020.entrust_price) ? ((parseFloat(P3020.entrust_price) + (parseInt(RS1050.price_step) * 0.001)).toFixed(RS1050.price_step === '10' ? 2 : 3)) : 1">
            <p class="p2"></p>
          </div>
        </div>
        <select class="con-select am-mb-0" v-model="P3020.entrust_prop">
          <template v-if="P3020.exchange_type === '1'">
            <option value="0">限价委托</option>
            <option value="U">五档即成剩撤</option>
            <option value="R">五档即成转限价</option>
          </template>
          <template v-if="P3020.exchange_type === '2'">
            <option value="0">限价委托</option>
          ...
          <template v-if="P3020.exchange_type === ''">
            <option value="0">限价委托</option>
          </template>
        </select>
        <div class="am-flexbox ui-formContainer am-flexbox-remind btn-border firmb-flexbox-buy am-mt-10 am-bg-white">
          <div class="firmbTags firmbTags-minus am-left-10"  @click="P3020.entrust_amount = P3020.entrust_amount.toString().length > 0 ? (parseFloat(P3020.entrust_amount) - 100 > 100 ? parseFloat(P3020.entrust_amount) - 100 : 100) : 100 ">
            <p class="p1"></p>
          </div>
          <div class=" borderNo am-flexbox-item">
                    <span class="ui-form-field">
                        <input v-model="P3020.entrust_amount" type="number" class="text_input am-ft-center " value="" placeholder="委托数量">
                    </span>
          </div>
          <div class="firmbTags am-right-10"  @click="P3020.entrust_amount = P3020.entrust_amount.toString().length > 0 ? parseFloat(P3020.entrust_amount) + 100 : 100">
            <p class="p2"></p>
          </div>
        </div>
        <div class="am-flexbox am-ft-smmm am-ft-lightgray am-mt-10">
          <div class="am-flexbox-item am-ft-left">可买 {{RS3010.enable_amount ? Math.transCNScientific(parseFloat(RS3010.enable_amount)) : ''}}</div>
          <div class="am-flexbox-item am-ft-left">可用 {{RS7050.fund_asset ? Math.transCNScientific(parseFloat(RS7050.fund_asset)) : ''}}</div>
        </div>
        <a @click="AS3020(this)" class="am-button am-mt-10 am-mb-15" am-mode="red">担保品买入</a>
      </div>
      <div class="sell-22" style="min-width: 95px">
        <ul class=" am-ft-smm border-bottom am-pb-5 sell-5 sell-8 am-relative">
          <li @click="this.P3020.entrust_price = this.RealData.sellPrice5" class="am-flexbox">
          ...
          <li @click="this.P3020.entrust_price = this.RealData.sellPrice4" class="am-flexbox">
          ...
          <li @click="this.P3020.entrust_price = this.RealData.sellPrice3" class="am-flexbox">
          ...
          <li @click="this.P3020.entrust_price = this.RealData.sellPrice2" class="am-flexbox">
          ...
          <li @click="this.P3020.entrust_price = this.RealData.sellPrice1" class="am-flexbox">
        ...
        <ul class="am-ft-smm am-mt-5 sell-5 sell-8">
          <li @click="this.P3020.entrust_price = this.RealData.buyPrice1" class="am-flexbox">
          ...
          <li @click="this.P3020.entrust_price = this.RealData.buyPrice2" class="am-flexbox">
          ...
          <li @click="this.P3020.entrust_price = this.RealData.buyPrice3" class="am-flexbox">
          ...
          <li @click="this.P3020.entrust_price = this.RealData.buyPrice4" class="am-flexbox">
          ...
          <li @click="this.P3020.entrust_price = this.RealData.buyPrice5" class="am-flexbox">
    ...
    <div class="container-list page-infinite-wrapper" v-el:wrapper :style='{ height: wrapper.height+"px"  }'>
      <ul class="itme am-cells" v-infinite-scroll='loadMore()' infinite-scroll-disabled='loading' infinite-scroll-distance='50'>
        <li class="am-flexbox border-bottom am-pd-15" v-for="item in RS4030"  v-bind:class="{ 'high_light': P3020.stock_code === item.stock_code }" @click="this.P3020.stock_code = item.stock_code">
          <div class="am-flexbox-item">
...
<script>
  ...
  export default{
    created () {
      ...
      this.AS7050()
      // 监听市场类别
      this.$watch('P3020.exchange_type', function (val, oldVal) {
        if (val === '') { // 没有市场类别
          // this.P3020['stock_account'] = ''
        } else {
          this.RS4070.map((account) => { // 信用账户
            if (account['exchange_type'] === val) {
              this.P3020['stock_account'] = account['stock_account']
            }
          })
        }
      })
      // 监听股票代码
      this.$watch('P3020.stock_code', function (val, oldVal) {
        this.StockList = []
        this.RealData = {}
            ...
            if ((list instanceof Array) && list.length === 1) {
              if (!list[0]['exchange_type']) {
                ...
                $Market(val).then((obj) => {
                  this.RealData = obj
                  // 买入价
                  this.P3020.entrust_price = obj.buyPrice1
                  // 国债没有涨跌停
                  if (list[0]['code_type'] === '0x1103' || list[0]['code_type'] === '0x1203') {
                    this.RealData.upPrice = '———'
                    this.RealData.downPrice = '———'
                  }
                  // 设置要查询可买数量的股票代码、市场内别、买入价
                  this.P3010['stock_code'] = val
                  this.P3010['exchange_type'] = this.P3020.exchange_type
                })
                this.P1050['stock_code'] = val
                this.AS1050()
                this.P3020['stock_name'] = list[0]['stock_name']
                this.P3020.exchange_type = list[0]['exchange_type']
              }
            }
          })
        } else {
      ...
      // 监听根据股票代码查询 - 可买数量
      this.$watch('P3020.entrust_price', function (val, oldVal) {
        if (val === '') {
          this.AS3010RESET()
        } else {
          this.P3010['entrust_price'] = this.P3020.entrust_price
          this.AS3010()
        }
      })
    ...
    vuex: {
      getters: {
        RS3010: state => state.all['301:0'],
        RS3020: state => state.all['302:0'],
