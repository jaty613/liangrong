  ...
  <section class="pd-bottom46">
    <section class="am-flexbox pd15 am-pt-10 border-top border-bottom am-pb-5 header-top am-bg-l">
      <div class="am-flexbox-item am-right-10">
        <select class="con-select" v-model="P7041.stock_account"  @change="changeStockAccount">
          <option v-for="option in RS4070"  v-bind:value="option.stock_account">
            {{$dic('exchange_type', option.exchange_type)}}—{{P7041.stock_account = option.stock_account}}
          </option>
        </select>
        <div class="con-code">
          <div class="ui-formContainer btn-border">
               <span class="ui-form-field">
                  <input v-model="P7041.stock_code" debounce="700" type="number" class="text_input" defaultvalue="股票名称/代码" value="" placeholder="请输入股票代码">
                  <a v-show="P7041.stock_code" @click="P7041.stock_code=''" class="close-ico bottom-10"></a>
              </span>
            <span class="share_name am-ft-gray">{{P7041.stock_name}}</span>
          </div>
          <!--浮出页面-->
          <div class="list-codeSug" v-show="(StockList.length > 0)">
            <ul class="select-results am-selectList" v-for="item in StockList">
              <li @click="P7041.stock_code = item.stock_code">
        ...
        <div class="am-flexbox ui-formContainer am-flexbox-remind btn-border firmb-flexbox-buy am-mt-10 am-bg-white">
          <div class="firmbTags firmbTags-minus am-left-10"  @click="P7041.entrust_price = !isNaN(P7041.entrust_price) ? ((parseFloat(P7041.entrust_price) - (parseInt(RS1050.price_step) * 0.001)).toFixed(RS1050.price_step === '10' ? 2 : 3)) : 1">
            <p class="p1"></p>
          </div>
          <div class=" borderNo am-flexbox-item">
                    <span class="ui-form-field">
                        <input v-model="P7041.entrust_price" type="number" class="text_input am-ft-center " value="" placeholder="买入价">
                    </span>
          </div>
          <div class="firmbTags am-right-10" @click="P7041.entrust_price = !isNaN(P7041.entrust_price) ? ((parseFloat(P7041.entrust_price) + (parseInt(RS1050.price_step) * 0.001)).toFixed(RS1050.price_step === '10' ? 2 : 3)) : 1">
          <p class="p2"></p>
          </div>
        </div>
        <select class="con-select am-mb-0" v-model="P7041.entrust_prop">
          <template v-if="P7041.exchange_type === '1'">
            <option value="0">限价委托</option>
            <option value="U">五档即成剩撤</option>
            <option value="R">五档即成转限价</option>
          </template>
          <template v-if="P7041.exchange_type === '2'">
            <option value="0">限价委托</option>
          ...
          <template v-if="P7041.exchange_type === ''">
            <option value="0">限价委托</option>
          </template>
        </select>
        <div class="am-flexbox ui-formContainer am-flexbox-remind firmb-flexbox-buy am-mt-10 btn-border am-bg-white">
          <div class=" borderNo am-flexbox-item">
                    <span class="ui-form-field">
                        <input v-model="P7041.entrust_amount" type="number" class="text_input am-ft-center " value="" placeholder="委托数量">
                    </span>
          </div>
          <div class="am-right-10 am-ft-lightgray am-ft-smmm">应还: {{RS7020.enable_amount ? Math.transCNScientific(parseInt(RS7020.enable_amount)) : '--'}}</div>
        </div>
        ...
        <a @click="AS7041(this)" class="am-button am-mt-10 am-mb-10" am-mode="red">买入</a>
      </div>
      <div class="sell-22" style="min-width: 95px">
        <ul class=" am-ft-smm border-bottom am-pb-5 sell-5 sell-8 am-relative">
          <li @click="this.P7041.entrust_price = this.RealData.sellPrice5" class="am-flexbox">
          ...
          <li @click="this.P7041.entrust_price = this.RealData.sellPrice4" class="am-flexbox">
          ...
          <li @click="this.P7041.entrust_price = this.RealData.sellPrice3" class="am-flexbox">
          ...
          <li @click="this.P7041.entrust_price = this.RealData.sellPrice2" class="am-flexbox">
          ...
          <li @click="this.P7041.entrust_price = this.RealData.sellPrice1" class="am-flexbox">
        ...
        <ul class="am-ft-smm am-mt-5 sell-5 sell-8">
          <li @click="this.P7041.entrust_price = this.RealData.buyPrice1" class="am-flexbox">
          ...
          <li @click="this.P7041.entrust_price = this.RealData.buyPrice2" class="am-flexbox">
          ...
          <li @click="this.P7041.entrust_price = this.RealData.buyPrice3" class="am-flexbox">
          ...
          <li @click="this.P7041.entrust_price = this.RealData.buyPrice4" class="am-flexbox">
          ...
          <li @click="this.P7041.entrust_price = this.RealData.buyPrice5" class="am-flexbox">
        ...
        <ul class="am-ft-smm am-mt-5 sell-5 sell-8 am-ml-5">
          <li class="am-flexbox">
            <div class="am-flexbox-item">涨停价&nbsp;&nbsp;&nbsp;<span class="am-ft-red">{{RealData.upPrice ? RealData.upPrice : '———'}}</span></div>
          </li>
          <li class="am-flexbox">
            <div class="am-flexbox-item">跌停价&nbsp;&nbsp;&nbsp;<span class="am-ft-green">{{RealData.downPrice ? RealData.downPrice : '———'}}</span></div>
          </li>
        </ul>
      </div>
    </section>
    ...
    <ul class="itme border-bottom head-list amTop346">
      <li class="am-flexbox itme-title  am-bg-l am-ft-smm am-ft-gray am-pd-15">
        <div class="am-flexbox-item">
          <p class="am-ft-15 am-ft-sm">融券负债</p>
        </div>
        <div class="am-right-10" style="min-width:80px !important;">
          <p class="am-ft-15 am-ft-sm">合约开仓数量</p>
        </div>
        <div class="am-flexbox-item text-center">
          <p class="am-ft-15 am-ft-sm">未还数量</p>
        </div>
        <div class="am-flexbox-item">
          <p class="am-ft-15 am-ft-sm">合约状态</p>
        </div>
      </li>
    </ul>
    <div class="container-list page-infinite-wrapper" v-el:wrapper :style='{ height: wrapper.height+"px"  }'>
      <ul class="itme am-cells" v-infinite-scroll='loadMore()' infinite-scroll-disabled='loading' infinite-scroll-distance='50'>
        <li class="am-flexbox border-bottom am-pd-15" v-for="item in RS7220" debounce="200" v-bind:class="{ 'high_light': P7041.stock_code === item.stock_code }" @click="this.P7041.stock_code = item.stock_code">
          <div class="am-flexbox-item">
            <p class="am-ft-15 am-ft-sm">{{item.stock_name}}</p>
            <p class="am-ft-11 am-ft-smm am-ft-gray">{{item.stock_code}}</p>
          </div>
          <div class="am-right-10" style="min-width:80px !important;">
            <p class="am-ft-15 am-ft-sm">{{Math.transCNScientific(parseInt(item.business_amount))}}</p>
          </div>
          <div class="am-flexbox-item text-center">
            <p class="am-ft-15 am-ft-sm">{{Math.transCNScientific(parseInt(item.transfer_return_amount))}}</p>
          </div>
          <div class="am-flexbox-item">
            <p class="am-ft-15 am-ft-sm">{{$dic('refcompact_status',item.compact_status)}}</p>
          </div>
        </li>
      </ul>
    </div>
...
<script>
  ...
  export default{
    created () {
      this.AS4070()
      // 监听市场类别
      this.$watch('P7041.exchange_type', function (val, oldVal) {
        if (val === '') { // 没有市场类别
          // this.P7041['stock_account'] = ''
        } else {
          this.RS4070.map((account) => { // 信用账户
            if (account['exchange_type'] === val) {
              this.P7041['stock_account'] = account['stock_account']
            }
          })
        }
      })
      // 监听股票代码
      this.$watch('P7041.stock_code', function (val, oldVal) {
        this.StockList = []
        this.RealData = {}
        if (val && val.length === 6) { // 输入完成
            ...
            if ((list instanceof Array) && list.length === 1) {
              if (!list[0]['exchange_type']) {
                ...
                $Market(val).then((obj) => {
                  ...
                  this.P7020['entrust_price'] = this.P7041.entrust_price
                })
                this.P1050['stock_code'] = val
                this.AS1050()
                this.P7041.exchange_type = list[0]['exchange_type']
                this.P7041.stock_name = list[0]['stock_name']
              }
            }
          })
        } else {
          ...
          // 重置内容
          this.P7041['exchange_type'] = ''
          this.P7041['stock_name'] = ''
          this.P7041['entrust_price'] = ''
          this.P7041['entrust_amount'] = ''
          // 重置要查询可还数量的股票代码
          this.P7020['stock_code'] = ''
        }
      })

      // 监听根据股票代码查询 - 可还数量
      this.$watch('P7020.stock_code', function (val, oldVal) {
        if (val === '') {
          this.AS7020RESET()
        } else {
          this.AS7020()
      ...
      this.$watch('P7041.entrust_amount', function (val, oldVal) {
        // 文本框 val 可还 this.RS7020.enable_amount
        this.sellToolActive = 0
        let amount = this.RS7020.enable_amount
      ...
      // 监听列表内容是否发生变化
      this.$watch('RS7220.length', function (val, oldVal) {
        this.loading = false
        if (val === oldVal) {
          this.loading = true
        }
    ...
    vuex: {
      getters: {
        RS7220: state => state.all['722:0'],
        RS4070: state => state.all['407:0'],
        RS7020: state => state.all['702:0'],
        RS1050: state => state.all['105:0']
      },
      actions: {
        AS1050: $AS('105:0'),
        AS7220: $AS('722:0'),
        RL7220: $RELOAD('722:0:0:4'),
        AS7041: $AS('704:1'),
        AS4070: $AS('407:0'),
        AS7020: $AS('702:0'),
        AS7020RESET: $AS('702:0:RESET')
      }
    },
    methods: {
      loadMore () {
        $P('722:0').position_str = this.RS7220[this.RS7220.length - 1].position_str
        if (!this.reload) {
      ...
      sellTools (m) {
        ...
        if (!isNaN(amount)) {
          let result = (amount / m)
          this.P7041.entrust_amount = (result <= 100) ? 100 : result - (result % 100)
        }
      },
      changeStockAccount () {
        // 重置内容
