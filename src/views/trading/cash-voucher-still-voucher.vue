  ...
  <section class="pd-bottom46">
    <section class="am-flexbox pd15 am-pt-10 border-top border-bottom am-pb-5 header-top am-bg-l">
      <div class="am-flexbox-item am-right-10">
        <select class="con-select am-mt-15 am-mb-15" v-model="P7080.stock_account" @change="changeStockAccount">
          <option v-for="option in RS4070"  v-bind:value="option.stock_account">
            {{$dic('exchange_type', option.exchange_type)}}—{{P7080.stock_account = option.stock_account}}
          </option>
        </select>
        <div class="con-code am-mb-15">
          <div class="ui-formContainer btn-border">
                     <span class="ui-form-field">
                        <input v-model="P7080.stock_code" debounce="700" type="number" class="text_input" defaultvalue="股票名称/代码" value="" placeholder="请输入股票代码">
                        <a v-show="P7080.stock_code" @click="P7080.stock_code=''" class="close-ico bottom-10"></a>
                    </span>
            <span class="share_name am-ft-gray">{{P7080.stock_name}}</span>
          </div>
          <!--浮出页面-->
          <div class="list-codeSug" v-show="(StockList.length > 0)">
            <ul class="select-results am-selectList" v-for="item in StockList">
              <li @click="P7080.stock_code = item.stock_code">
        ...
        <div class="am-flexbox ui-formContainer am-flexbox-remind firmb-flexbox-buy am-mt-10 btn-border am-bg-white">
          <div class=" borderNo am-flexbox-item">
                    <span class="ui-form-field">
                        <input v-model="P7080.occur_amount" type="number" class="text_input am-ft-center " value="" placeholder="委托数量">
                    </span>
          </div>
        ...
        <ul class="am-flexbox listLateral am-mt-30 am-ft-smm am-mb-20">
        ...
        <a @click="AS7080(this)" class="am-button am-mt-10 am-mb-10" am-mode="green">还券</a>
      </div>
      <div class="sell-22" style="min-width: 95px">
        <ul class=" am-ft-smm border-bottom am-pb-5 sell-5 sell-8 am-relative">
          <li @click="this.P7080.entrust_price = this.RealData.sellPrice5" class="am-flexbox">
          ...
          <li @click="this.P7080.entrust_price = this.RealData.sellPrice4" class="am-flexbox">
          ...
          <li @click="this.P7080.entrust_price = this.RealData.sellPrice3" class="am-flexbox">
          ...
          <li @click="this.P7080.entrust_price = this.RealData.sellPrice2" class="am-flexbox">
          ...
          <li @click="this.P7080.entrust_price = this.RealData.sellPrice1" class="am-flexbox">
        ...
        <ul class="am-ft-smm am-mt-5 sell-5 sell-8">
          <li @click="this.P7080.entrust_price = this.RealData.buyPrice1" class="am-flexbox">
          ...
          <li @click="this.P7080.entrust_price = this.RealData.buyPrice2" class="am-flexbox">
          ...
          <li @click="this.P7080.entrust_price = this.RealData.buyPrice3" class="am-flexbox">
          ...
          <li @click="this.P7080.entrust_price = this.RealData.buyPrice4" class="am-flexbox">
          ...
          <li @click="this.P7080.entrust_price = this.RealData.buyPrice5" class="am-flexbox">
    ...
    <ul class="itme border-bottom head-list amTop312">
    ...
    <div class="container-list page-infinite-wrapper" v-el:wrapper :style='{ height: wrapper.height+"px"  }'>
      <ul class="itme am-cells" v-infinite-scroll='loadMore()' infinite-scroll-disabled='loading' infinite-scroll-distance='50'>
        <li class="am-flexbox border-bottom am-pd-15" v-for="item in RS7220" debounce="200" v-bind:class="{ 'high_light': P7080.stock_code === item.stock_code }" @click="this.P7080.stock_code = item.stock_code">
          <div class="am-flexbox-item">
...
<script>
  ...
  export default{
    created () {
      this.AS4070()
      // 监听市场类别
      this.$watch('P7080.exchange_type', function (val, oldVal) {
        if (val === '') { // 没有市场类别
          // this.P7080['stock_account'] = ''
        } else {
          this.RS4070.map((account) => { // 信用账户
            if (account['exchange_type'] === val) {
              this.P7080['stock_account'] = account['stock_account']
            }
          })
        }
      })
      // 监听股票代码
      this.$watch('P7080.stock_code', function (val, oldVal) {
        this.StockList = []
        this.RealData = {}
        if (val && val.length === 6) { // 输入完成
            ...
            if ((list instanceof Array) && list.length === 1) {
              if (!list[0]['exchange_type']) {
                ...
                $Market(val).then((obj) => {
                  this.RealData = obj
                  // 买入价
                  this.P7080.entrust_price = obj.buyPrice1
                  // 国债没有涨跌停
                  ...
                  this.P7020['exchange_type'] = this.P7080.exchange_type
                  this.P7020['entrust_price'] = this.P7080.entrust_price
                })
                this.P1050['stock_code'] = val
                this.AS1050()
                this.P7080.exchange_type = list[0]['exchange_type']
                this.P7080.stock_name = list[0]['stock_name']
              }
            }
          })
        } else {
          ...
          // 重置内容
          this.P7080['exchange_type'] = ''
          this.P7080['stock_name'] = ''
          this.P7080['occur_amount'] = ''
          // 重置要查询可还数量的股票代码
      ...
      this.$watch('P7080.occur_amount', function (val, oldVal) {
    ...
    methods: {
      ...
      sellTools (m) {
        ...
        if (!isNaN(amount)) {
          let result = (amount / m)
          this.P7080.occur_amount = (result <= 100) ? 100 : result - (result % 100)
        }
      },
      changeStockAccount () {
        // 重置内容
